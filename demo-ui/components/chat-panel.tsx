...
export function ChatPanel({ submitted, submittedNote, draft, onDraftChange, onCompose, events, status, runId, finalState }: {
  ...
  const statusMessage = !hasActiveRun && status === "idle"
    ? "Không có run đang hoạt động. Tạo run mới để tiếp tục."
    : status === "running"
      ? "Đang xử lý các bước của workflow"
      : status === "completed"
        ? `Workflow kết thúc: ${finalState?.replaceAll("_", " ") ?? "completed"}`
        : status === "failed"
          ? "Workflow gặp lỗi kỹ thuật"
          : "Đang khởi tạo run";
  ...
  return <section className="panel chat-panel" id="chat" aria-labelledby="chat-title">
    ...
    <div className="chat-messages" aria-live="polite">
      {!submitted ? <div className="chat-empty"><span className="chat-empty-mark" aria-hidden="true">↗</span><h3>Bắt đầu một phiên quan sát</h3><p>Nhập ghi chú, thiết lập thông số và theo dõi các bước mà backend phát ra.</p></div> : <>
        ...
        <div className="message status-message"><span className="message-role">Workflow · {hasActiveRun ? runId.slice(0, 8) : "no active run"}</span><p>{statusMessage}</p>
          {recent.length ? <ul>{recent.map((event) => <li key={event.event_id}><span aria-hidden="true">{event.type === "state.failed" || event.type === "workflow.failed" ? "!" : event.type.endsWith("completed") ? "✓" : "•"}</span> {event.type} {event.state ?? ""}</li>)}</ul> : <small>{hasActiveRun ? "Chờ event từ server…" : "Run chưa được tạo."}</small>}
        </div>
      </>}
    </div>
